...
def predict_distance(x: np.ndarray, scaler, poly, theta: np.ndarray) -> float:
    """
...
def objective_function(x: np.ndarray, target_distance: float, scaler, poly, theta: np.ndarray) -> float:
    """
...
def plot_2d_cut(target_distance: float, scaler, poly, 
                ...
                fixed_x: list[float] | None = None, variable_index: int = 1, variable_name: str = "firing_angle"):
    """
    ...
    for value in values:
        x = np.asarray(fixed_x, dtype=float).copy()
...
def plot_3d_surface(scaler, poly, 
                    theta: np.ndarray, bounds: Bounds = BOUNDS, 
                    fixed_x: list[float] | None = None, x_index: int = 0, 
                    y_index: int = 1, x_name: str = "release_angle", y_name: str = "firing_angle"):
    """
    ...
    Mantém as demais variáveis fixas.
    """
    ...
    for i in range(X.shape[0]):
        for j in range(X.shape[1]):
            x_config = np.asarray(fixed_x, dtype=float).copy()
